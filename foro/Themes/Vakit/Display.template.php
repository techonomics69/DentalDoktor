...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		echo ' <div class="topbar_post">
		       <div>', $message['time'], '</div><span>', !empty($message['counter']) ? $txt['reply_noun'] . ' #' . $message['counter'] : '', '</span> 
               </div>
		...
		if (!$message['member']['is_guest'])
		{
			...
			if (!isset($context['disabled_fields']['posts']))
				echo '
								<li class="postcount"><b>', $txt['member_postcount'], ':</b> ', $message['member']['posts'], '</li>';

			// Is karma display enabled?  Total or +/-?
			if ($modSettings['karmaMode'] == '1')
				echo '
								<li class="karma"><b>', $modSettings['karmaLabel'], '</b> ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</li>';
			...
			if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
				echo '
								<li class="gender"><b>', $txt['gender'], ':</b> ', $message['member']['gender']['image'], '</li>';
		...
		if ($context['can_quote'])
			echo '
				<a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '">', $txt['reply_quote'], '</a>';
			echo'
				</div>';
	}
